...
n_Cdc42 = 5000;
n_GAP = 0;
P3 = [0.018]; % value of lambda3*dt
k2b = [0.01,0.015,0.02,0.025,0.03,0.035,0.045,0.055,0.065,0.1,...
    6.85,6.95,7.05,7.15,7.25,7.35,7.45,7.55,7.65,7.75,7.85,7.95,8,8.1,8.3];
        ...
        for v = 1:numel(n_BemGEF)
            for w = 1:numel(n_GAP)
                for m = 1:numel(P3)
                    for n = 1:numel(k2b)
                        curr_seed  = j;
                        curr_fileprefix = sprintf('Cdc42_%g-Bem_%g-GAP_%g-P3_%g-k2b_%g-%d',n_Cdc42(u),n_BemGEF(v),n_GAP(w),P3(m),k2b(n),j);
                        smoldyn_cfg(curr_fileprefix,directory,tstop,samplingrate,n_Cdc42(u),n_BemGEF(v),n_GAP(w),P3(m),k2b(n),curr_seed);
